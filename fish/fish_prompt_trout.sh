...
function fish_prompt
  ...
  if set -q VIRTUAL_ENV
    echo -n (set_color -o blue)"("(basename $VIRTUAL_ENV)")"(set_color -o normal)
  end

  # Show current user in green parentheses
  set user_info "$green($USER)$normal"
  ...
  echo -n -s $user_info $git_info $prompt
end
